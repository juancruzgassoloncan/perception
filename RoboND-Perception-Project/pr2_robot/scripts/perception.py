...
def pcl_callback(pcl_msg):
    ...
    pcl_clean = statistical_filter(pcl_raw,k_neighbors=20,scale_f=0.05)
    # Voxel Grid Downsampling
    pcl_dsample = voxel_grid_filter(pcl_clean, l_size=0.01)
    
    # PassThrough Filter
    pcl_filt = passThrough_filter(pcl_dsample,
                                 axis='z', axis_min=0.57, axis_max=1.1)
    pcl_filt = passThrough_filter(pcl_filt,
                                 axis='y', axis_min=-0.46, axis_max=0.46)
    pcl_filt = passThrough_filter(pcl_filt,
                                     axis='x', axis_min=0.4, axis_max=1)
    ...
    cluster_idx = clustering_eu_dist(white_cloud, tol=0.02, minCsize=50, maxCsize=25000)
